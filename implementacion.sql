...
ID_DIR NUMBER(9) CONSTRAINT CENTRO_DE_OPERACIONES_ID_DIR_NONULO NOT NULL CONSTRAINT CENTRO_DE_OPERACIONES_ID_DIR_FK_DIRECCION_ID REFERENCES DIRECCION(ID)
);


/*FALTA CREAR RELACIÓN DE N A N CON PEDIDO??? Yo creo que es con producto*/
...
ID_C_OPS NUMBER(9) CONSTRAINT ALMACEN_ID_C_OPS_NONULO NOT NULL CONSTRAINT ALMACEN_ID_C_OPS_FK_CENTRODEOPERACIONES_ID REFERENCES CENTRO_DE_OPERACIONES(ID)
);
...
ID_C_OPS NUMBER(9) CONSTRAINT OFICINA_ID_C_OPS_NONULO NOT NULL CONSTRAINT OFICINA_ID_C_OPS_FK_CENTRODEOPERACIONES_ID REFERENCES CENTRO_DE_OPERACIONES(ID)
);
...
DNI_JEFE VARCHAR(9) CONSTRAINT DNI_JEFE_NONULO NOT NULL CONSTRAINT EMPLEADO_DNI_JEFE_FK_EMPLEADO_DNI_JEFE REFERENCES EMPLEADO(DNI) /*AQUÍ HE PUESTO QUE REFERENCIA A SU PROPIA ENTIDAD*/
);
...
ESTADO_PEDIDO_ID NUMBER(2) CONSTRAINT PEDIDO_ESTADOPEDIDOID_NONULO NOT NULL CONSTRAINT PEDIDO_ESTADOPEDIDOID_FK_ESTADO_PEDIDO_ID REFERENCES ESTADO_PEDIDO(ID),
NUM_DATO_BANCARIO VARCHAR(24) CONSTRAINT PEDIDO_NUMDATOBANCARIO_NONULO NOT NULL CONSTRAIT PEDIDO_NUMDATOBANCARIO_FK_DATO_BANCARIO_NUM REFERENCES DATO_BANCARIO(NUM),
DNI_EMPLEADO VARCHAR(9) CONSTRAINT PEDIDO_DNIEMPLEADO_NONULO NOT NULL CONSTRAINT PEDIDO_DNIEMPLEADO_FK_EMPLEADO_DNI REFERENCES EMPLEADO(DNI),
DNI_USUARIO VARCHAR(9) CONSTRAINT PEDIDO_DNIUSUARIO_NONULO NOT NULL CONSTRAINT PEDIDO_DNIUSUARIO_FK_USUARIO_DNI REFERENCES USUARIO(DNI),
DIR_ID NUMBER(9) CONSTRAINT PEDIDO_DIR_ID_NONULO NOT NULL CONSTRAINT PEDIDO_DIR_ID_FK_DIRECCION_ID REFERENCES DIRECCION(ID)
);
...
ID_PEDIDO NUMBER(15) CONSTRAINT RECLAMACION_QUEJA_PEDIDOID_NONULO NOT NULL CONSTRAINT RECLAMACION_QUEJA_IDPEDIDO_FK_PEDIDO_ID REFERENCES PEDIDO(ID),
DNI_EMPLEADO VARCHAR(9) CONSTRAINT RECLAMACION_QUEJA_DNI_EMPLEADO_NONULO NOT NULL CONSTRAINT RECLAMACION_QUEJA_DNI_EMPLEADO_FK_EMPLEADO_DNI REFERENCES EMPLEADO(DNI)
);

CREATE TABLE DISPONIBILIDAD(
ID_MODELO NUMBER(9) CONSTRAINT DISPONIBILIDAD_ID_MODELO_NONULO NOT NULL CONSTRAINT DISPONIBILIDAD_ID_MODELO_FK_MODELO_ID REFERENCES MODELO(ID),
ID_ALMACEN NUMBER(9) CONSTRAINT DISPONIBILIDAD_ID_ALMACEN_NUNULO NOT NULL CONSTRAINT DISPONIBILIDAD_ID_ALMACEN_FK_ALMACEN_IF REFERENCES ALMACEN(ID),
